    ...
    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
      {users?.map((user: User) => {
        ...
        return (
          <div
            key={user.id}
            className="bg-white dark:bg-card-main rounded-2xl border border-slate-200 dark:border-card-border-secondary shadow-sm p-6 relative group transition-all hover:shadow-md"
          >
            {/* Top Row: Avatar & Name */}
            ...
            <div className="grid grid-cols-2 gap-3 pt-4 border-t border-slate-50 dark:border-white/5">
              <button
                onClick={() => !isRestricted && onEdit(user)}
                disabled={isRestricted}
                className={`flex items-center justify-center gap-2 py-2.5 rounded-xl border text-sm font-bold transition-all ${
                  isRestricted
                    ? "opacity-20 border-slate-400 dark:border-card-border-primary text-slate-800 cursor-not-allowed"
                    : "border-slate-200 dark:border-white/10 text-slate-600 dark:text-zinc-300 hover:bg-slate-50 dark:hover:bg-white/5 cursor-pointer"
                }`}
              >
                <UserIcon size={16} />
                Profile
